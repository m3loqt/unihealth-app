...
export default function PatientReferralDetailsScreen() {
  ...
  // Check for existing feedback when referral data loads
  useEffect(() => {
    const checkExistingFeedback = async () => {
      if (referralData?.id && referralData?.status === 'completed') {
        try {
          const feedbackExists = await databaseService.checkReferralFeedbackExists(referralData.id);
  ...
  const handleSubmitFeedback = async () => {
    if (!referralData?.id || !feedbackStars) return;
    ...
    try {
      ...
      // Get doctor data
      const doctorData = await databaseService.getDoctorById(referralData.assignedSpecialistId, false);
      const doctorName = doctorData ? `${doctorData.firstName || ''} ${doctorData.lastName || ''}`.trim() : 'Unknown Doctor';
      
      // Get clinic data
      const clinicData = await databaseService.getClinicByIdForDisplay(referralData.referringClinicId);
      const clinicName = clinicData?.name || referralData.referringClinicName || 'Unknown Clinic';
      ...
      Alert.alert(
        'Error',
        error.message === 'Rating already submitted for this referral'
        ...
        <View style={styles.moreMenuOverlay}>
          ...
          />
           <View style={styles.moreMenuContainer}>
             ...
             {referralData?.status === 'completed' && !hasExistingFeedback && (
               <TouchableOpacity
                 style={styles.moreMenuItem}
                 onPress={() => {
                   setShowMoreMenu(false);
